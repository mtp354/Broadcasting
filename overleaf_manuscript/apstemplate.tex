...
&\qquad 0\le p\le 1, \nonumber
\end{align}
where $X$, $Y$, and $Z$ are the Pauli operators. The presence of noise can degrade the fidelity of the output state at the receivers, making it crucial to analyze the performance of the protocol under these conditions and to explore methods for improving the protocol's resilience such as quantum error correction.
%
...
\end{align}
Here, the labels $A,B$ indicate who is in possession of which qubit; these will be omitted going forward, but the structure remains the same. The target state at each receiver is the equatorial state $\ket{\psi_{\mathrm{target}}} = \frac{1}{\sqrt{2}}\!\left(e^{i\theta}\ket{0} + e^{-i\theta}\ket{1}\right)$. \\
...
\end{align}
where each sender is kept as a native $(N{+}1)$-dimensional qudit rather than embedded into qubits, removing the unused basis states $k>N$ from the simulation.

Starting from the pure resource state $\rho_0 = \ket{\Psi^{(M,N)}}\!\bra{\Psi^{(M,N)}}$ (optionally followed by the encoding isometry $I_a \otimes \Enc^{\otimes N}$ when QEC is enabled), the protocol is applied as a sequence of completely-positive maps:
\begin{enumerate}
    ...
    \item Fourier-basis measurement on each sender qudit, realized as projectors $\Pi_n^{(j)} = \ket{u_n}\!\bra{u_n}$, with the outcome $\bar n$ either sampled from the Born distribution or set to a fixed sequence for deterministic studies.
...
Storing $\rho$ requires $O(D^2)$ complex numbers, and each Kraus operation costs $O(D^2)$ work per local subsystem (implemented as tensor contractions over the relevant qubit/qudit axes). Without QEC the cost grows as $(N+1)^{2M} \cdot 4^{N}$, which is tractable for the protocol sizes considered in this work ($M\le 3$, $N\le 4$). When QEC is enabled the receiver register expands by a factor of $2^{4N}$, so the cost scales as $(N+1)^{2M} \cdot 4^{5N}$; this becomes the dominant bottleneck and motivates the Monte Carlo alternative described next. The exact method is deterministic up to the (optional) measurement-outcome RNG and is used throughout as the ground-truth reference against which the sampling estimator is benchmarked.
...
%\begin{region}
This research was supported, in part, by a grant of computer time from the City University of New York High Performance Computing Center under NSF Grants CNS-0855217, CNS-0958379 and ACI-1126113. The code developed for this work is available at Zenodo XXXXXXX.
%\end{region}
