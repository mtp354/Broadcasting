#!/usr/bin/env python3
"""Hardware experiment entry point. No subcommand submits implicitly."""
...
from broadcasting.experiments import (
    attach_job, experiment_status, collect_experiment, plan_experiment,
    prepare_experiment, read_config, submit_experiment,
)
...
def main(argv=None):
    ...
    try:
        if args.command == "plan":
            result = plan_experiment(read_config(args.config))
            if not args.full:
                result = {key: value for key, value in result.items() if key != "repeats"}
        elif args.command == "prepare":
            prepared = prepare_experiment(read_config(args.config), args.run_dir)
            ...
            result = experiment_status(args.run_dir)
        elif args.command == "submit":
            result = {"submitted_job_ids": submit_experiment(args.run_dir)}
        elif args.command == "collect":
            result = {"saved_results": [str(path) for path in collect_experiment(args.run_dir)]}
        else:
