"""Historical provenance, real error arithmetic and resumable MC checkpoints."""
...
def test_original_history_retains_actual_grid_parameters_and_printed_errors(tmp_path):
    study = conv.load_historical_convergence(archive_dir=tmp_path)
    ...
    rows = re.findall(r"n_samples=\s*(\d+)\s+per-receiver error=\[([^\]]+)\]", study.historical["source"]["printed_output"])
    assert study.historical["errors_per_receiver"] == [[float(x) for x in v.split()] for _, v in rows]
    assert study.historical["errors_per_receiver"][-1] == [0.00127751, 0.00120541]
...
def test_collection_persists_effective_seeds_counts_reference_and_resumes(monkeypatch, tmp_path):
    ...
    conv.collect_convergence_repeats(study, seeds=[1, 2], sample_counts=[50, 100], batch_dir=batch)
    assert seen == [("exact", 0, None)] + [("sample", seed, count) for seed in [1, 2] for count in [50, 100]]
    loaded = conv.load_historical_convergence(archive_dir=tmp_path)
    ...
    checksums = {p: conv._digest(p) for p in batch.glob("*.json")}
    conv.collect_convergence_repeats(loaded, seeds=[1, 2], sample_counts=[50, 100], batch_dir=batch)
    assert len(seen) == 5
    assert checksums == {p: conv._digest(p) for p in batch.glob("*.json")}
    with pytest.raises(ValueError, match="Batch settings changed"):
        conv.collect_convergence_repeats(loaded, seeds=[1, 2], sample_counts=[50], batch_dir=batch)
    with pytest.raises(ValueError, match="previously used"):
        conv.collect_convergence_repeats(loaded, seeds=[1, 2], sample_counts=[50], batch_dir=tmp_path / "duplicate")
...
def test_interruption_retains_finished_points_and_resumes(monkeypatch, tmp_path):
    ...
    with pytest.raises(RuntimeError, match="interrupted"):
        conv.collect_convergence_repeats(study, repeats=1, seeds=[1], sample_counts=[50, 100], batch_dir=tmp_path / "resume")
    loaded = conv.load_historical_convergence(archive_dir=tmp_path)
    assert len(loaded.repetitions[0]["points"]) == 1
    assert not loaded.repetitions[0]["complete"]
    failures.clear()
    conv.collect_convergence_repeats(loaded, repeats=1, seeds=[1], sample_counts=[50, 100], batch_dir=tmp_path / "resume")
...
def test_loader_rejects_changed_reference_and_excludes_other_physical_settings(monkeypatch, tmp_path):
    ...
    study = conv.load_historical_convergence(archive_dir=tmp_path)
    batch = conv.collect_convergence_repeats(study, repeats=1, sample_counts=[50], batch_dir=tmp_path / "matching")
    exact_path = batch / "exact.json"
    ...
    with pytest.raises(ValueError, match="Incompatible convergence"):
        conv.load_historical_convergence(archive_dir=tmp_path)
    manifest_path = batch / "study.json"
    manifest = json.loads(manifest_path.read_text())
    manifest["config"]["thetas"] = [1.0]
    manifest_path.write_text(json.dumps(manifest))
    assert conv.load_historical_convergence(archive_dir=tmp_path).repetitions == []


def test_plot_restores_log_log_and_keeps_figure_open_with_new_curve(monkeypatch, tmp_path):
    _fake_backends(monkeypatch)
    study = conv.load_historical_convergence(archive_dir=tmp_path)
    conv.collect_convergence_repeats(study, repeats=1, sample_counts=[50, 100], batch_dir=tmp_path / "new")
    loaded = conv.load_historical_convergence(archive_dir=tmp_path)
    fig = conv.plot_convergence(loaded, output_path=tmp_path / "convergence.png")
    ax = fig.axes[0]
    assert ax.get_xscale() == ax.get_yscale() == "log"
    assert len(ax.lines) == 3
    assert list(ax.lines[0].get_xdata()) == study.sample_counts
    assert np.asarray(ax.lines[1].get_ydata()) == pytest.approx([0.04, 0.02])
    assert plt.fignum_exists(fig.number)
    assert (tmp_path / "convergence.png").exists()
    plt.close(fig)
    with pytest.raises(ValueError, match="PNG only"):
        conv.plot_convergence(study, output_path=tmp_path / "convergence.pdf")
    plt.close("all")
...
def test_tiny_real_collection_round_trips_actual_backend(tmp_path):
    ...
    batch = conv.collect_convergence_repeats(study, repeats=1, seeds=[1], sample_counts=[2], batch_dir=tmp_path / "tiny")
    reference, digest = conv._reference(batch, study.config)
    run, errors = conv._measurement(batch / "run_seed1_n2.json", study.config, reference, digest)
...
def test_original_seed_zero_curve_matches_current_physical_simulator(tmp_path):
    """A bounded 50-trajectory regression makes the historical overlay auditable."""
    ...
    assert np.allclose(errors, study.historical["errors_per_receiver"][0], rtol=0, atol=5e-9)
