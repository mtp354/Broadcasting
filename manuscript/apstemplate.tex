...
% The restored historical convergence plot is one fixed-seed size sweep;
% independent repeats must remain distinct from this rounded notebook evidence.
...
For independent trajectories, scalar sample means have standard errors proportional to $n_s^{-1/2}$ when their variance is finite. Figure~\ref{qec_crossover} compares one saved sampling run with exact simulations and the independently checked logical polynomial. Figure~\ref{fig:mc_convergence} combines a separate historical sample-size sweep reconstructed from archived notebook summaries with four completed repetitions using independent seeds 1--4. For each seed it displays the receiver errors $E_i=\int_0^1|\widehat F_i(p)-F_i^{\mathrm{exact}}(p)|\,dp$ and their total $E_1+E_2$, evaluated by the trapezoidal rule on the common probability grid. Receiver 1, receiver 2, and total errors are overlaid in red, blue, and black, respectively. Each curve retains its seed across sample sizes, so points within a curve are coupled estimates. The historical seed-0 summaries and the four repetitions remain separate without averaging across seeds; no fitted convergence exponent or repetition-based confidence interval is reported.
...
In simulation, $p$ directly specifies an independent depolarizing channel. On hardware we instead sweep an inserted idle delay $\tau$. A delay exposes qubits to device noise but does not reproduce the single-parameter depolarizing model. Transpiler optimization level $\ell\in\{0,1,2,3\}$ is a separate compilation setting that can change layout, routing, and gate simplification; it is not a physical channel parameter. We use the backend time step $dt$ recorded with a run, or an explicitly documented historical value, when expressing integer delays in seconds. Delay sweeps and optimization settings do not by themselves diagnose noise correlations.

The reported circuits use \texttt{SamplerV2}. No explicit dynamical-decoupling sequence or zero-noise extrapolation is documented for the displayed runs. Where supported by the backend target, the current execution path compiles a parameterized circuit once per sender-angle tuple and then binds the delay sweep. This construction order does not establish a particular physical schedule: transpilation and hardware timing constraints determine instruction placement, and historical records do not preserve every compiled schedule.

Figure~\ref{QEC Fidelity} uses a separate single-qubit memory benchmark. It prepares a state with $R_y(\theta)$ followed by $R_z(\phi)$, optionally encodes it, inserts the delay, recovers and decodes if encoded, reverses the preparation, and measures the return probability. The displayed encoded runs perform worse than the unencoded runs over their sampled delays, but the records were collected on different dates. This comparison concerns this implemented circuit and its overhead, rather than a general limitation of QEC.

\begin{figure}[!tbp]
    \centering
    \includegraphics[width=\linewidth]{qec513_delay_sweep_idle.png}
    \caption{Standalone single-qubit memory benchmark on IBM Kingston: encoded $[[5,1,3]]$ runs at optimization levels 0 and 3 (13 May 2026), and bare runs at levels 3 (18 May) and 0 (18 June). All four runs use $8192$ shots per point. They form an exploratory comparison across dates; the manifest documents the historical $dt$ value used for time conversion. The noise-free reference is the saved ideal-circuit result. These are not QEC-enhanced broadcasting data.}
    \label{QEC Fidelity}
\end{figure}
...
\begin{figure*}[!t]
    ...
    \caption{Historical exhaustive-feedforward circuit for $M{=}1$, $N{=}2$ without QEC. The sender qudit ($a_0, a_1$; two qubits encoding a qutrit) is initialized together with two receiver qubits ($r_0, r_1$) in the resource state $\ket{\Psi^{(1,2)}}$. After a parametric delay $\tau$, the sender phase gate $U_a(\theta)$ and inverse-DFT rotation $F_d^\dagger$ are applied, followed by a computational-basis measurement. Three conditional blocks (one per measurement outcome $n\in\{0,1,2\}$) each apply the corresponding phase correction $P(-2\pi n/3)$ to both receivers. Finally, $R_z(-\phi)$ and Hadamard gates rotate each receiver into the equatorial fidelity-measurement basis. The current default replaces these three exhaustive branches with two bitwise conditional blocks, as described in the text.}
    \label{fig:circuit}
\end{figure*}

A schematic of the historical exhaustive-feedforward Qiskit circuit for the $M{=}1$, $N{=}2$ protocol without QEC is shown in Fig.~\ref{fig:circuit}. The circuit is built in six stages:
...
\subsection{Analysis of archived readouts}
The historical archive contains 16 distinct broadcasting jobs comprising 819 per-angle, per-delay joint fidelity-register histograms, and five distinct standalone memory jobs. We additionally include the completed scaling and delay campaigns: six broadcasting jobs containing 39 case records and 399 histograms. The combined broadcasting collection therefore contains 22 jobs and 1218 histograms. Duplicate saves of the same observation are counted once, while separate cases within a job remain distinct. The source inventory records input hashes and historical attribution evidence; counts are analyzed separately for each job, case, angle tuple, and delay.
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=\linewidth]{figure_03_qec_crossover.png}
    \caption{Average receiver fidelity as a function of depolarizing probability $p$ for the $M{=}1$, $N{=}2$ protocol with and without $[[5,1,3]]$ quantum error correction compared against Monte Carlo trajectory sampling estimates. The inset resolves the low-noise crossing at $p_\star=(3-\sqrt6)/4$. The saved curves use $\alpha=1/\sqrt3$, $\theta=\pi/4$; local depolarizing fidelity is independent of this pure-target choice. The sampling curve is one saved run, not a convergence study.}
    \label{qec_crossover}
\end{figure}

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\linewidth]{figure_04_delay_repeats.png}
    \caption{Receiver fidelity versus inserted delay for five separate unencoded $M=1,N=2$ runs at optimization level 3, grouped into two panels by backend. (a) Marrakesh: the historical sweep saved on 8 September 2026 ($4096$ shots per point) overlaid with the three repetitions of \texttt{broadcasting-delay-01}, each with $10{,}000$ shots at 121 delays spanning $0$--$24\,\mu\mathrm{s}$. (b) Kingston: the historical sweep saved on 18 May 2026 ($10{,}000$ shots per point). Red and blue identify receiver 1 and receiver 2, respectively, and black denotes their mean within each run. Both panels use the same three-entry legend. All curves are solid lines without markers at opacity $0.8$; receiver curves and their means remain separate between runs. The $dt=4$ ns conversion is recorded for the Marrakesh runs and inferred from a same-device, same-day memory-job output for the historical Kingston run.}
    \label{qiskit_delay}
\end{figure*}

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\linewidth]{figure_05_hardware_scaling.png}
    \caption{Unencoded hardware observations at zero added delay and optimization level 3. The 55 points comprise 36 case/repetition observations from \texttt{broadcasting-scaling-01} ($M=1,2,3$, $N=1,2,3,4$, three repetitions, $8192$ shots per case), the three zero-delay observations from \texttt{broadcasting-delay-01}, and 16 eligible historical job/angle observations. Colour identifies sender count $M$ and marker shape identifies the backend. Each point is the receiver mean for one job, case and sender-angle tuple; its vertical bar spans the minimum and maximum receiver marginals. Bars describe receiver heterogeneity, not statistical uncertainty. Small horizontal offsets separate observations at the same integer $N$. The campaigns use IBM Marrakesh; the combined collection spans backends, dates, shot counts, and circuit variants and is not pooled into a controlled scaling curve.}
    \label{protocol_scale}
\end{figure*}

Figure~\ref{qec_crossover} compares saved exact bare and encoded simulations, a saved Pauli-trajectory estimate, and the analytical curves. The exact curves agree with their predicted fidelities. The sampling points provide a fixed-sample numerical comparison; the independently constructed stabilizer oracle and analytical factorization provide checks that do not rely on agreement between the two simulation paths alone. The low-noise QEC advantage ends at $p_\star\simeq0.1376$, as derived above.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.8\linewidth]{figure_06_sampling_convergence.png}
    \caption{Monte Carlo-to-exact convergence for the encoded $M=1,N=2$ protocol, $\alpha=1/\sqrt2$, $\theta=0$, and sender outcome 0. For each seed, red and blue lines show the integrated absolute fidelity errors for receivers 1 and 2 over 21 equally spaced depolarizing probabilities in $[0,1]$, and a black line shows their sum (Total). Each curve covers the same ten sample counts from 50 to $10^5$. Historical seed-0 curves are reconstructed from notebook errors printed to eight decimal places; their original sampled fidelity grids were not retained. Four completed repetitions using independent seeds 1--4 are computed from archived sampled grids and their exact references. A seed is held fixed across sample counts within each curve, and seeds are overlaid without averaging. All curves are solid and marker-free, with opacity 0.8. The gray $n_s^{-1/2}$ reference is anchored at the first historical total-error point and is not a fitted uncertainty estimate.}
    \label{fig:mc_convergence}
\end{figure*}

Figure~\ref{qiskit_delay} overlays the historical unencoded $M=1,N=2$ Marrakesh delay sweep and the three completed Marrakesh campaign repetitions in the left panel, with the historical Kingston sweep in the right panel. The campaign varies the sender phase between repetitions and retains each run separately. Its receiver-mean fidelities at zero added delay are $0.9023$, $0.8965$, and $0.9087$ for repetitions 1--3, respectively. The fidelity is already below one near zero added delay, so errors outside the inserted idle interval contribute materially. Differences between campaign traces can reflect phase and execution differences as well as shot uncertainty. The historical Kingston receiver traces differ and exhibit nonmonotonic delay structure. A device-specific coherence-time explanation would require the calibration and layout at execution; no contemporaneous calibration supports assigning a particular $T_1$ range to that historical run.

At zero added delay in the historical Kingston run, the two receiver fidelities are $0.8941$ and $0.9257$. The worst-receiver interval is $[0.8870,0.9008]$; the joint fidelity is $0.8493$ with interval $[0.8422,0.8562]$. The success covariance is $0.02163$ with delta-method interval $[0.01882,0.02444]$, and the paired difference $F_1-F_2=-0.03160$ has interval $[-0.03840,-0.02480]$. Thus the recorded receiver imbalance is larger than the conditional shot uncertainty at this point, while its reproducibility across executions remains undetermined.

The historical Marrakesh sweep saved on 8 September 2026 (run timestamp 15:31:50) uses $4096$ shots per point. At zero added delay its receiver mean is $0.8474$, worst-receiver fidelity is $0.8350$ with interval $[0.8216,0.8475]$, and joint fidelity is $0.7473$ with interval $[0.7338,0.7604]$. Its success covariance is $0.02936$ with interval $[0.02407,0.03466]$. This run remains a separate observation because device and circuit history differ from the May data. Positive measured covariance can arise from shared preparation, feedforward, readout, or drift as well as physical noise correlations, and cannot distinguish those mechanisms by itself.

The historical Kingston optimization-level-3 receiver-mean trace has a selected period of $0.504\,\mu\mathrm{s}$ with frequency-bin spacing $0.0413\,\mu\mathrm{s}^{-1}$. A sinusoid at this peak explains only $6.6\%$ of the linearly detrended variance. The level-0 trace gives $0.440\,\mu\mathrm{s}$ and $4.9\%$. Both peaks have only about two to three samples per cycle and lie near the Nyquist limit. Time conversion for these May traces uses a same-device, same-day $dt$ inference documented in the manifest. The September trace instead uses its recorded $dt$; its dominant period is $24.2\,\mu\mathrm{s}$ over a $24\,\mu\mathrm{s}$ observation window, so it is unresolved against smooth drift. These spectral summaries do not establish a reproducible oscillation period or a dynamical-decoupling mechanism.

Figure~\ref{protocol_scale} combines the 36 completed scaling-campaign observations with historical observations and the zero-delay points from the three delay-campaign repetitions. The scaling campaign covers all combinations of $M=1,2,3$ and $N=1,2,3,4$ in each of three jobs on IBM Marrakesh, with $8192$ shots per case and a separately retained sender-angle tuple per repetition. Measurements at the same $(M,N)$ remain separate. The combined display spans backends, dates, shot counts, and circuit versions, so its points do not constitute a controlled scaling curve. Even within a matched cohort, a decrease in local fidelity with larger circuits can arise from accumulating independent gate, routing, idle, and readout errors. The prediction of $N$-independent local fidelity applies only to one idealized receiver channel per qubit, not to the full hardware circuit. The existing observations therefore do not establish correlated physical noise.

The library implements a complete QEC-enhanced broadcasting circuit, validated in simulation. The reported hardware QEC data in Fig.~\ref{QEC Fidelity} are the standalone encoded-memory benchmark; a complete QEC-enhanced broadcasting hardware experiment has not been established by the archived data.
...
The historical hardware records and completed broadcasting campaigns show circuit overhead, receiver asymmetry, and delay-dependent structure. The standalone encoded-memory benchmark does not demonstrate an encoded fidelity advantage for its sampled settings, and the heterogeneous broadcasting observations cannot isolate a noise mechanism or establish a controlled scaling law. Reanalysis recovers joint fidelity, worst-receiver performance, success covariance, paired asymmetry, and descriptive delay spectra. These statistics establish features of the recorded readouts; attribution to a physical noise mechanism requires matched circuit models and controlled repeats with preserved calibration, compiled circuits, and aligned shot records. The updated figures incorporate the six completed campaign jobs alongside the historical archive; all plots and statistics are reconstructed from saved results.

\section{Acknowledgements}
This research was supported by the National Science Foundation under the grant Collaborative Research: NeTS: Medium 2504622. Additionally it was supported in part, by a grant of computer time from the City University of New York High Performance Computing Center under NSF Grants CNS-0855217, CNS-0958379 and ACI-1126113. A public archival identifier for the code and data remains to be assigned before submission. Additionally we are grateful for the feedback and advice of IBM's Dr. Olivia Lanes and Dr. Nick Bronn regarding our hardware fidelity results.
