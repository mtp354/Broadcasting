...
\end{align}
Where $\rho_1, \rho_2$ are the density matrices of the output and target state respectively. For pure states this reduces to 
\begin{align}
...
\begin{align}
    \ket{\psi_{\mathrm{target}}} = \frac{1}{\sqrt{2}}\!\left(e^{i\theta}\ket{0} + e^{-i\theta}\ket{1}\right) = \frac{1}{\sqrt{2}}\!\left(\ket{0} + e^{-2i\theta}\ket{1}\right).
\end{align}
...
QEC codes improve fidelity up to a certain level of noise. The fidelity for a pure input state to the depolarizing channel with depolarizing probability $p$ is given by Eq.~(\ref{fidelity_eqn}).
...
\end{align}
which matches the numerically observed crossover in Fig.~\ref{qec_crossover} to within simulation precision. The cruder estimate obtained by truncating $p_L(p)$ at leading order, $p_L(p)\approx 10p^2$, and equating $1-\tfrac23(10p^2)$ to $F_{\mathrm{bare}}(p)$ gives only $p\approx 0.1$; the difference from $p_\star\approx0.1376$ comes from the $O(p^3)$ and higher terms of $p_L(p)$, not from any two-qubit errors being (mis)corrected -- for the minimum-weight decoder used here, every one of the $90$ distinct-qubit weight-two Pauli patterns already induces a nontrivial logical error and is counted correctly by $p_L(p)$. We independently verified Eq.~(\ref{eq:pL}) by brute-force enumeration in code (\texttt{logical\_error\_probability\_bruteforce} in \texttt{broadcasting/simulation.py}), which reproduces the closed form without reusing the recovery-map implementation under test.

We restrict the plotted range to $p\in[0,1]$, the physically meaningful range of the channel convention used here; note that at $p=3/4$ the channel is completely depolarizing ($\eta=0$) and the encoded and unencoded fidelities coincide at $F=1/2$, visible as the second crossing in Fig.~\ref{qec_crossover}.
%\end{region}
...
\begin{enumerate}
    ...
    \item Accumulate the empirical density matrix on the decoded space, $\hat\rho = \tfrac{1}{n_s}\sum_{s=1}^{n_s} \ket{\psi_s}\!\bra{\psi_s}$, after $n_s$ trajectories (or, when only fidelity is required, accumulate the scalar per-trajectory fidelity directly and skip the density-matrix estimate entirely).
\end{enumerate}
...
Each trajectory itself costs $O(D)$ in memory and $O(N\cdot D)$ in time for the Pauli applications and per-block recovery. However, if the empirical density matrix $\hat\rho$ is accumulated (as opposed to a scalar fidelity), its storage and the $n_s$ outer-product accumulations cost an additional $O(d_{\mathrm{dec}}^2)$ and $O(n_s\, d_{\mathrm{dec}}^2)$ respectively, where $d_{\mathrm{dec}} = (N{+}1)^M\cdot 2^N$ is the decoded dimension; this is not negligible once $\hat\rho$ is needed rather than a scalar observable, and we report it separately rather than folding it into a single asymptotic cost. In practice we measure wall-clock time and peak memory directly for the reported $(M,N)$ rather than relying on the asymptotic count alone. Because trajectories are independent, the statistical fluctuation in each scalar observable (here the per-receiver fidelity) decays as $1/\sqrt{n_s}$; we verify this scaling in Fig.~\ref{Sampling Error Convergence} by measuring the area between the sampled and exact fidelity curves $F(p)$ for the $M{=}1$, $N{=}2$ protocol with QEC and fitting the convergence exponent rather than assuming it. We use $n_s$ in the range $10^2$--$10^5$ and demonstrate rapid convergence to the complete density matrix method for large samples. We see also in Fig.~4 the close fit of the Monte Carlo sampling method fidelity to the exact density matrix case with error correction for $M=1, N=2$; the exact logical polynomial $p_L(p)$ derived above (and validated independently by brute-force enumeration) provides a ground truth for both that does not depend on either implementation.
\\
...
The $[[5,1,3]]$ error correction is implemented in the circuit using four shared ancilla qubits and Qiskit's dynamic-circuit primitives. For each five-qubit block the circuit appends three stages, described below.
...
We first verify the correctness of the exact density matrix and Monte Carlo sampling implementations by comparing their fidelity estimates for the $M{=}1$, $N{=}2$ protocol with QEC across a range of depolarizing probabilities $p$. As shown in Fig.~\ref{qec_crossover}, the two methods produce consistent results, with the sampling estimates converging to the exact curve as the number of trajectories increases. The plot also includes a zoomed view of the crossover region where the QEC-enhanced protocol loses its advantage over the unencoded case, which occurs at $p \approx 0.137$. This crossover point is higher than the rough estimate of $p \approx 0.1$ derived from the failure probability of the $[[5,1,3]]$ code, indicating that the effective noise after correction is less severe than the naive binomial approximation as expected. The exact density matrix method is used as the ground truth for benchmarking the sampling estimator, and the agreement between the two validates the use of Monte Carlo sampling for estimating protocol performance under noise, especially for larger receiver counts where the exact method becomes computationally infeasible. \\
%

We then implement the protocol on IBM quantum hardware using Qiskit, as described in the Methods section. Fig.~\ref{qiskit_delay} shows the fidelity of the output state with respect to the input state as a function of delay time $\tau$ for the $M{=}1$, $N{=}2$ protocol without QEC. The results indicate that the fidelity decreases with increasing delay time, which is expected due to decoherence effects on the hardware. However, even at short delay times, the fidelity is significantly below 1, suggesting that other sources of noise and imperfections in the hardware such as imperfect gate fidelity and readout error are also contributing to the degradation of the output state. This highlights the challenges of implementing quantum protocols on current noisy intermediate-scale quantum (NISQ) devices and underscores the importance of error mitigation and correction techniques for improving performance. Additionally, "spikes" of low fidelity are observed at certain delay times, which may be due to specific noise patterns or fluctuations in the hardware performance. Similar structures have been observed in other experiments and are referred to as "drop-outs" \cite{niu2024multi}. We describe these as \emph{visually} quasi-periodic; we have not yet quantified the periodicity with an autocorrelation or periodogram analysis, and we have not applied or scheduled any explicit dynamical-decoupling sequence in this run, so DD is not established as the cause of these features. Additionally, the protocol should be symmetric between receivers however in Fig.~\ref{qiskit_delay} we see clear and persistent differences in fidelity between Receiver 1 and Receiver 2. A possible cause of this could be the heterogeneity of coherence times between qubits which range significantly. For this particular device the T1 time ranges from $10.11-434.27\mathrm{\mu s}$\cite{javadi2024qiskit}. We have not yet collected enough repeated data to determine whether this asymmetry is a reproducible device effect or is within run-to-run statistical variation. \\
%

Lastly, we explore the feasibility of this protocol as the number of receivers scales. Our earlier analysis of independent depolarizing channels being the dominant source of decoherence breaks down as we increase the protocol size. This is demonstrated by the rapidly decreasing average receiver fidelity at $\tau=0$ for $M=1$. Independent, but accumulating, per-gate and readout errors in the larger transpiled circuits required for bigger $M,N$ can by themselves produce this decline, since local receiver fidelity being independent of $N$ is a property of the idealized single-channel-per-receiver model, not of the full compiled hardware circuit -- larger $M,N$ require deeper state preparation, more routing, and more idle/measured qubits, independent of any correlation between receivers. The results in Fig.~\ref{protocol_scale} are therefore consistent with accumulated, independent hardware noise as circuit size grows; correlated noise across receivers remains one possible contributing explanation but is not established by this local-average-fidelity measurement alone (see Conclusion for what additional measurements would be needed). Separately, we note that Fig.~\ref{QEC Fidelity} is a standalone encoded-memory benchmark (a single logical qubit stored in one $[[5,1,3]]$ block with no broadcasting, sender measurement, or byproduct correction involved), distinct in scope from the unencoded broadcasting circuits behind Figs.~\ref{qiskit_delay} and~\ref{protocol_scale}; we have not implemented or benchmarked a complete QEC-enhanced broadcasting circuit combining both. This is a significant challenge to the feasibility of this protocol for many practical use cases, indicating that hardware improvements and sophisticated QEC protocols are likely needed.
...
\section{Conclusion}
%\begin{region}
We have taken an existing quantum broadcasting protocol and studied its behavior under noise using a range of numerical techniques. We have then extended this protocol to include QEC with which we have demonstrated the region for with QEC is advantageous. Lastly, we have implemented this protocol on hardware, where we have analyzed the impact of QEC and found that current hardware used was not able to implement QEC advantageously. Our hardware results also indicate significant scaling challenges of this broadcasting protocol to larger network sizes, consistent with accumulated independent hardware noise as circuit size grows; distinguishing this from genuinely correlated receiver noise would require joint all-receiver fidelity, pairwise covariance, and a comparison against a backend-calibrated independent noise model of the same transpiled circuit, none of which we have yet measured. Most surprisingly, our hardware idle time results revealed a visually quasi-periodic "drop-out" phenomenon, that has been observed before but is not widely understood; we have not quantified its periodicity or established dynamical decoupling as its cause.
%

Broadcasting of quantum states using shared prior entanglement is a candidate resource for distributed quantum computing and quantum networking, though our results indicate significant challenges in current implementation. The privacy property we establish is specific and limited: the classical correction messages reveal only the aggregate sender phase $\Phi=\sum_j\theta_j$ and not any individual $\theta_j$; this is not a general cryptographic security or authentication guarantee, and we make no claim of Byzantine-agreement or device-independent security. On the experimental side, hardware idle noise demonstrates fidelity degradation consistent with accumulating independent gate and readout errors as circuit size grows, which by itself is enough to make large multipartite entangled states difficult to scale effectively; whether additional correlated-noise structure also contributes remains an open question given the measurements available. Additionally, readout and gate fidelities are too low for basic QEC implementation to be effective. On the theory side, optimized broadcasting protocols are needed that incorporate natively tailored QEC techniques and are robust against both independent and correlated noise.

%\end{region}

\section{Acknowledgements}
%\begin{region}
This research was supported by the National Science Foundation under the grant Collaborative Research: NeTS: Medium 2504622. Additionally it was supported in part, by a grant of computer time from the City University of New York High Performance Computing Center under NSF Grants CNS-0855217, CNS-0958379 and ACI-1126113. The code developed for this work is available at \color{red}Zenodo XXXXXXX. \color{black}Additionally we are grateful for the feedback and advice of IBM's Dr. Olivia Lanes and Dr. Nick Bronn regarding our hardware fidelity results.
%\end{region}
