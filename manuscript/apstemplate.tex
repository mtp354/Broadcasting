...
For independent trajectories, scalar sample means have standard errors proportional to $n_s^{-1/2}$ when their variance is finite. Figure~\ref{qec_crossover} compares one saved sampling run with exact simulations and the independently checked logical polynomial. Figure~\ref{fig:mc_convergence} restores a separate historical sample-size sweep from archived notebook summaries. Its error is $\sum_i\int_0^1|\widehat F_i(p)-F_i^{\mathrm{exact}}(p)|\,dp$, evaluated by the trapezoidal rule on the common probability grid. The same seed is used across sample sizes, so these points are coupled estimates rather than independent repetitions. No convergence exponent or repetition-based confidence interval is inferred from this single trace.
...
We reanalyze 16 distinct broadcasting jobs comprising 819 per-angle, per-delay joint fidelity-register histograms, and five distinct standalone memory jobs. Duplicate saves of the same job are counted once. The source inventory records input hashes and historical attribution evidence; counts are analyzed separately for each job, angle tuple, and delay.
...
\begin{minipage}[t]{0.48\textwidth}
    ...
    \caption{Receiver fidelity versus inserted delay for the unencoded $M=1,N=2$ protocol on IBM Kingston, optimization level 3, collected 18 May 2026 with $10{,}000$ shots per point. The $dt=4$ ns conversion is inferred from a Kingston memory-job output on the same date; it was not archived with this broadcasting job.}
    \label{qiskit_delay}
\end{minipage}
\end{figure*}
...
\begin{figure*}[!t]
    ...
    \caption{Existing unencoded hardware observations at zero added delay, restricted to optimization level 3. The horizontal axis is the number of receivers $N$, the vertical axis is receiver fidelity, and colour identifies the number of senders $M$. Each point represents one job and sender-angle tuple: its dot is the receiver mean and its vertical bar spans the minimum and maximum receiver marginals. Bars describe receiver heterogeneity, not statistical uncertainty. Small horizontal offsets separate observations at the same $N$; they do not represent noninteger network sizes. The saved collection spans backends, dates, shot counts, and circuit variants, so it remains exploratory rather than a controlled scaling cohort.}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.62\linewidth]{mc_sampling_convergence.png}
    \caption{Historical Monte Carlo-to-exact convergence for the encoded $M=1,N=2$ protocol, $\alpha=1/\sqrt2$, $\theta=0$, and sender outcome 0. The ordinate sums the two receivers' integrated absolute fidelity errors over 21 equally spaced depolarizing probabilities in $[0,1]$. Sample counts range from 50 to $10^5$, using seed 0 throughout. Points are reconstructed from archived notebook errors printed to eight decimal places; the original sampled fidelity grids were not retained. The dotted $n_s^{-1/2}$ reference is anchored at the first point and is not a fitted uncertainty estimate.}
    \label{fig:mc_convergence}
\end{figure*}

Figure~\ref{qiskit_delay} shows the existing unencoded $M=1,N=2$ IBM Kingston delay sweep at optimization level 3. The fidelity is already below one near zero added delay, so errors outside the inserted idle interval contribute materially. The receiver traces also differ and exhibit nonmonotonic delay structure. A device-specific coherence-time explanation would require the calibration and layout at execution; no contemporaneous calibration supports assigning a particular $T_1$ range to this run.
...
The Kingston optimization-level-3 receiver-mean trace has a selected period of $0.504\,\mu\mathrm{s}$ with frequency-bin spacing $0.0413\,\mu\mathrm{s}^{-1}$. A sinusoid at this peak explains only $6.6\%$ of the linearly detrended variance. The level-0 trace gives $0.440\,\mu\mathrm{s}$ and $4.9\%$. Both peaks have only about two to three samples per cycle and lie near the Nyquist limit. Time conversion for these May traces uses a same-device, same-day $dt$ inference documented in the manifest. The September trace instead uses its recorded $dt$; its dominant period is $24.2\,\mu\mathrm{s}$ over a $24\,\mu\mathrm{s}$ observation window, so it is unresolved against smooth drift. These spectral summaries do not establish a reproducible oscillation period or a dynamical-decoupling mechanism.

Figure~\ref{protocol_scale} displays the available optimization-level-3 observations against receiver count, with sender count distinguished by colour. Measurements at the same $(M,N)$ remain separate. They span backends, dates, shot counts, and circuit versions, so they do not constitute a controlled scaling curve. Even within a matched cohort, a decrease in local fidelity with larger circuits can arise from accumulating independent gate, routing, idle, and readout errors. The prediction of $N$-independent local fidelity applies only to one idealized receiver channel per qubit, not to the full hardware circuit. The existing observations therefore do not establish correlated physical noise.
