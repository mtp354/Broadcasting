...
Figure~\ref{qiskit_delay} overlays three unencoded $M=1,N=2$ delay sweeps per backend, with Marrakesh on the left and Kingston on the right. The sender phases match across backends in each of the three runs, and every sweep is retained separately. In run order, the receiver-mean fidelities at zero added delay are $0.9023$, $0.8965$, and $0.9087$ for Marrakesh, and $0.8516$, $0.8739$, and $0.8632$ for Kingston. The fidelity is already below one near zero added delay, so errors outside the inserted idle interval contribute materially. Differences between traces can reflect phase, backend, and execution differences as well as shot uncertainty. A device-specific coherence-time explanation would require the calibration and layout associated with each execution.
