...
% The previous convergence image repeated an effective seed. A new, archived
% independent-seed study is required before restoring a convergence figure.
...
For independent trajectories, scalar sample means have standard errors proportional to $n_s^{-1/2}$ when their variance is finite. Figure~\ref{qec_crossover} compares one saved sampling run with exact simulations and the independently checked logical polynomial; it is a fixed-sample comparison from which no convergence exponent is inferred.
...
\begin{figure}[!tbp]
    \centering
    \includegraphics[width=\linewidth]{qec513_delay_sweep_idle.pdf}
...
\begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{qec vs no qec vs sampling.pdf}
...
\begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{delay time vs fidelity 121 opt3.pdf}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\linewidth]{fidelity scaling hardware.pdf}
    \caption{Existing unencoded hardware observations at zero added delay. Each point represents a job and sender-angle tuple, with backend, date, $(M,N)$, optimization level, and shot count retained. Points show receiver means and horizontal bars span receiver marginals; these bars represent receiver heterogeneity, not statistical uncertainty. The heterogeneous collection includes IBM Fez, Marrakesh, and Kingston and historical and current circuit variants; it is exploratory evidence, not a controlled size-scaling experiment.}
