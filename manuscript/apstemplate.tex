...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\linewidth,height=0.68\textheight,keepaspectratio]{figure_04_delay_repeats.png}
    \caption{Receiver fidelity versus inserted delay for five separate unencoded $M=1,N=2$ runs at optimization level 3. The first two panels show the historical Kingston sweep saved on 18 May 2026 ($10{,}000$ shots per point) and Marrakesh sweep saved on 8 September 2026 ($4096$ shots per point). The remaining panels show the three Marrakesh repetitions of \texttt{broadcasting-delay-01}, each with $10{,}000$ shots at 121 delays spanning $0$--$24\,\mu\mathrm{s}$. Each panel identifies its sender phase; receiver curves and their mean remain separate between runs. Shaded bands are marginal 95\% Wilson shot intervals and exclude device drift. The $dt=4$ ns conversion is recorded for the Marrakesh runs and inferred from a same-device, same-day memory-job output for the historical Kingston run.}
    \label{qiskit_delay}
\end{figure*}
...
\begin{figure*}[!t]
    ...
    \caption{Monte Carlo-to-exact convergence for the encoded $M=1,N=2$ protocol, $\alpha=1/\sqrt2$, $\theta=0$, and sender outcome 0. The ordinate sums the two receivers' integrated absolute fidelity errors over 21 equally spaced depolarizing probabilities in $[0,1]$. Each curve covers the same ten sample counts from 50 to $10^5$. Historical seed-0 points are reconstructed from notebook errors printed to eight decimal places; their original sampled fidelity grids were not retained. Four completed repetitions using independent seeds 1--4 are computed from archived sampled grids and their exact references. A seed is held fixed across sample counts within each curve. The dotted $n_s^{-1/2}$ reference is anchored at the first historical point and is not a fitted uncertainty estimate.}
    \label{fig:mc_convergence}
\end{figure*}

Figure~\ref{qiskit_delay} places two historical unencoded $M=1,N=2$ delay sweeps alongside the three completed Marrakesh campaign repetitions. The campaign varies the sender phase between repetitions and retains each run separately. Its receiver-mean fidelities at zero added delay are $0.9023$, $0.8965$, and $0.9087$ for repetitions 1--3, respectively. The fidelity is already below one near zero added delay, so errors outside the inserted idle interval contribute materially. Differences between campaign traces can reflect phase and execution differences as well as shot uncertainty; the shaded intervals quantify only the last of these. The historical Kingston receiver traces differ and exhibit nonmonotonic delay structure. A device-specific coherence-time explanation would require the calibration and layout at execution; no contemporaneous calibration supports assigning a particular $T_1$ range to that historical run.
